...
class WMSDemoDataAgent {
  ...
  async seedDemoData() {
    ...
    if (regRes.status === 201) {
      ...
      // Already registered, login to fetch token
    ...
    if (mgrReg.status === 201) {
      ...
      console.log(`  - Tài khoản Quản Lý Kho đã tồn tại: ${managerEmail}`);
    }
    ...
    if (staffReg.status === 201) {
      ...
      console.log(`  - Tài khoản Nhân Viên Kho đã tồn tại: ${staffEmail}`);
    }
    ...
    const demoProducts = [
      { sku: 'LAP-DELL-XPS15', name: 'Laptop Dell XPS 15 (i7/16GB/512GB)', category: 'Máy Tính', unit: 'Cái', price: 42500000, minQuantity: 5 },
      { sku: 'MACBOOK-PRO-16', name: 'MacBook Pro 16 inch M3 Max', category: 'Máy Tính', unit: 'Cái', price: 79900000, minQuantity: 3 },
      { sku: 'MON-LG-274K', name: 'Màn Hình LG UltraFine 27" 4K IPS', category: 'Màn Hình', unit: 'Cái', price: 12800000, minQuantity: 8 },
      { sku: 'KB-LOGI-MXKEYS', name: 'Bàn Phím Cơ Logitech MX Keys S', category: 'Phụ Kiện', unit: 'Cái', price: 3250000, minQuantity: 10 },
      { sku: 'MOU-LOGI-MXM3', name: 'Chuột Không Dây Logitech MX Master 3S', category: 'Phụ Kiện', unit: 'Cái', price: 2850000, minQuantity: 10 },
      { sku: 'PHONE-SS-S24U', name: 'Điện Thoại Samsung Galaxy S24 Ultra', category: 'Điện Thoại', unit: 'Cái', price: 31990000, minQuantity: 5 },
      { sku: 'TAB-APPLE-M4', name: 'Máy Tính Bảng iPad Pro 11" M4 Chip', category: 'Máy Tính Bảng', unit: 'Cái', price: 28500000, minQuantity: 4 },
    ];

    const productMap: Record<string, string> = {};

    for (const prod of demoProducts) {
      const res = await this.request('POST', '/products', prod, adminToken);
      if (res.status === 201) {
        productMap[prod.sku] = res.data.id;
        console.log(`  ✓ Đã tạo Sản Phẩm: [${prod.sku}] ${prod.name} - Giá: ${prod.price.toLocaleString('vi-VN')} đ`);
      } else {
    ...
    // Step 5: Create & Complete Import Receipts (Stock Increment & Log Trail)
    ...
    // Step 6: Create Export Receipts (Completed & Pending)
    ...
    // Export 2 (PENDING)
    const expRes2 = await this.request('POST', '/movements', {
      type: 'EXPORT',
      note: 'Xuất hàng cho văn phòng đại diện Hà Nội (Chờ duyệt)',
      items: [
        { productId: productMap['MACBOOK-PRO-16'], quantity: 2, price: 83000000 },
      ],
    }, adminToken);

    if (expRes2.status === 201) {
      console.log(`  ⏳ Đã khởi tạo Phiếu Xuất (Chờ duyệt): [${expRes2.data.code}] - 2 MacBook Pro 16"`);
    }

    // Step 7: Fetch Dashboard Stats Verification
    console.log('\n======================================================');
    ...
    if (statsRes.status === 200) {
      ...
      console.log(`  • Số phiếu chờ duyệt (PENDING): ${s.pendingMovementsCount} phiếu`);
